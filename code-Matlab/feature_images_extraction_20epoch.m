...
nepochs(i)=floor(epochs(i)/20)
tlabels(i)=floor(epochs(i)/tepochs);
extra_epochs=mod(epochs(i),tepochs);
epindx=1;


for c=eeg_channel_pos;
      for ep=1:size(EEG_data.data,3);
                    [s(:,:,c,ep), fi, ti]=spectrogram(EEG_data.data(c,:,ep), floor(win_length), overlap,1024,fs);
%                     
...
% 
       ...
       extra_epochs=mod(epochs(i),tepochs);
       epindx=1;
       for ep=1:nepochs(i):((size(EEG_data.data,3)-extra_epochs));
                   for c=eeg_channel_pos;
                                  ...
                                  axis xy
                                  axis off
                                  saveas(gcf,[fni,subject_id,'_',stimduration,'_',frequncey,'_',num2str(epindx),'_',num2str(c),'_',cell2mat(event_id),'_itc.png']);
                                  close all;
                                  ...
                                  axis xy
                                  %                     axis off;
                                  saveas(gcf,[fne,subject_id,'_',stimduration,'_',frequncey,'_',num2str(epindx),'_',num2str(c),'_',cell2mat(event_id),'_ersp.png']);
                                  labels_stft_ersp(i,epindx,c)=event_label;
                                  close all;
                                 ...
                                 figure;
                                 st=mean(abs(s(freq_ind,time_ind,c,ep:ep+nepochs-1)),4);
                                 imagesc(ti(time_ind)*1000, fi(freq_ind), mag2db(st));
                                 axis xy
%                                  axis off
                                 saveas(gcf,[fns,subject_id,'_',stimduration,'_',frequncey,'_',num2str(epindx),'_',num2str(c),'_',cell2mat(event_id),'_STFT.png']);
                                 close all;
                                 labels_stft_ersp(i,epindx,c)=event_label;
...
%%
 save('imagefeaturelabels.mat', 'label_itc','labels_stft_ersp','epochs','tlabels','nepochs'); % save both variables to a file
